...
public final class FavoritesNode extends FilterNode implements Index {
    ...
    public PasteType getDropType(Transferable t, int action, int index) {
        ...
        for (DataObject dataObject : dos) {
            if (! FavoritesActions.Add.isAllowed(dataObject))
                return null;
    }
    ...
    private Index getIndex() {
        return getOriginal().getCookie(Index.class);
    }
    
    @Override
    public Image getIcon (int type) {
        return ImageUtilities.loadImage("org/starbeans/modules/corelite/resources/actionSelect.gif"); // NOI18N
    }
    ...
    public Action[] getActions(boolean context) {
        return new Action[] {FavoritesActions.addOnFavoritesNode()};
    }
        ...
        private Action [] createActionsForFavoriteFolder (Action [] arr) {
            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                if (!added && ((arr[i] instanceof CopyAction) || (arr[i] instanceof CutAction))) {
                    added = true;
                    //STARBEANS
                    newArr.add(org.openide.util.actions.SystemAction.get(org.openide.actions.OpenLocalExplorerAction.class));
                    newArr.add(FavoritesActions.remove());
                    newArr.add(null);
                }
        ...
        private Action [] createActionsForFolder (Action [] arr) {
            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                if (!added && ((arr[i] instanceof CopyAction) || (arr[i] instanceof CutAction))) {
                    ...
                    //STARBEANS
                    newArr.add(org.openide.util.actions.SystemAction.get(org.openide.actions.OpenLocalExplorerAction.class));
                    newArr.add(FavoritesActions.add());
                    newArr.add(null);
                }
                newArr.add(arr[i]);
            }
    ...
    private static class FavoritesPasteType extends PasteType {
        ...
        public Transferable paste() throws IOException {
            FavoritesActions.Add.addToFavorites(Arrays.asList(dos));
            return null;
        }
