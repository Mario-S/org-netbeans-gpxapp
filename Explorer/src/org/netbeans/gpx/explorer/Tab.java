...
import org.netbeans.gpx.explorer.FavoritesNode;
...
//            WindowManagerImpl.deferredPerformer().putRequest(this, null);
    ...
    * 2) Changes of IDE settings, namely delete confirmation settings */
    ...
    public static synchronized Tab findDefault() {
        if(DEFAULT == null) {
            //STARBEANS
            TopComponent tc = WindowManager.getDefault().findTopComponent("explorer"); // NOI18N
            if(DEFAULT == null) {
    ...
    */
    //STARBEANS made public, see ProjectUtilities
    public boolean selectNode (DataObject obj, Node root) {
        Node parent = findClosestNode(obj, root, true);
        Node node = parent;
        //STARBEANS select first marked subnode if any
        ...
        if(child!=null) {
            ...
            for(int n=0; n<nodes.length; n++)
                if(nodes[n]!=null) {
                    Object o = nodes[n].getValue("marked");
            ...
            // Pure convention, if none of them are marked, open the first
            // one if the parent is marked.
            if(found==false && nodes.length>0) {
                Object o = parent.getValue("marked"); //parent
                if (o != null && o.equals(Boolean.TRUE))
                    node = nodes[0];                
            }
        }
    ...
    */
    private static Node findDataObject (Node node, DataObject obj) {
        ...
        for (int i = 0; i < arr.length; i++) {
            DataShadow ds = arr[i].getCookie (DataShadow.class);
            if ((ds != null) && (obj == ds.getOriginal())) {
                return arr[i];
            } else {
                DataObject o = arr[i].getCookie (DataObject.class);
                if ((o != null) && (obj == o)) {
                    return arr[i];
                }
            }
    ...
    * to keep the uniquennes. */
    ...
    protected void doSelectNode (DataObject obj) {
        ...
        if (selectNode (obj, root)) {
            ...
            try {
                FavoritesActions.Add.addToFavorites(Collections.singletonList(DataObject.find(file)));
    ...
    private static FileObject chooseFileObject(FileObject file) {
        ...
        chooser.setDialogTitle(NbBundle.getBundle(FavoritesActions.class).getString ("CTL_DialogTitle"));
        chooser.setApproveButtonText(NbBundle.getBundle(FavoritesActions.class).getString ("CTL_ApproveButtonText"));
        ...
        if ( option == JFileChooser.APPROVE_OPTION ) {
            ...
            if (!selectedFile.exists()) {
                String message = NbBundle.getMessage(FavoritesActions.class,"ERR_FileDoesNotExist",selectedFile.getPath());
                String title = NbBundle.getMessage(FavoritesActions.class,"ERR_FileDoesNotExistDlgTitle");
